...
class _VideoCallScreenState extends State<VideoCallScreen> {
  ...
  // NEW: Track if the call was actually connected (volunteer joined)
        ...
        onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) {
          ...
          if (mounted) {
            setState(() {
              ...
              // NEW: Mark that connection was established
              _wasCallConnected = true;
            });

            // Volunteer joined, cancel timer & notify
            _waitTimer?.cancel();
            if (widget.isBlindUser) {
              _flutterTts.stop();
              _flutterTts.speak("Volunteer has joined.");
  ...
  Future<void> _endCall() async {
    ...
    // NEW: If call was connected (both parties joined), mark as COMPLETED
    if (_wasCallConnected) {
      try {
        safePrint("Marking call as COMPLETED...");
        const String mutation = r'''
          mutation UpdateCallStatus($id: ID!, $status: CallStatus!) {
            updateCall(input: {id: $id, status: $status}) {
              id
              status
            }
          }
        ...
        safePrint("✅ Call status updated to COMPLETED");
      } catch (e) {
        safePrint("❌ Error updating call status to completed: $e");
      }
    }
