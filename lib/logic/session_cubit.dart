...
class SessionCubit extends Cubit<SessionState> {
  GoogleGenAI? _genAI; // Changed to nullable to prevent crashes if key is missing

  // --- SERVICES ---
  FlutterVoiceEngine? _voiceEngine;
  LiveSession? _geminiSession;
  final MLKitService _mlKitService = MLKitService(); // Initialize ML Kit Service
  ...
  Future<void> _connectToGeminiLive() async {
    if (_genAI == null) {
      emit(state.copyWith(error: "Gemini API Key not initialized", isError: true));
      return;
    }
          ...
          systemInstruction: Content(
            parts: [
              ...
              "You are an AI assistant for visually impaired users. "
                  "Describe what you see in the camera images clearly. "
  ...
  void _handleGeminiLiveMessage(LiveServerMessage message) {
    try {
      if (message.serverContent?.modelTurn?.parts != null) {
        for (final part in message.serverContent!.modelTurn!.parts!) {
          if (part.inlineData != null && part.inlineData!.mimeType.startsWith('audio/')) {
            ...
            try {
              final Uint8List amplifiedAudio = _amplifyAudio(audioData, amplificationFactor: 12.0);
      ...
      if ((message.serverContent?.turnComplete ?? false) || (message.serverContent?.generationComplete ?? false)) {
  ...
  Future<void> _initCameraForOnlineMode() async {
    await _initCameraWithFormat(Platform.isAndroid ? ImageFormatGroup.yuv420 : ImageFormatGroup.bgra8888);
  }

  Future<void> _initCameraForOfflineMode() async {
    await _initCameraWithFormat(Platform.isAndroid ? ImageFormatGroup.nv21 : ImageFormatGroup.bgra8888);
  }
  ...
  void _startImageSendTimer() {
    _imageSendTimer?.cancel();
    _imageSendTimer = Timer.periodic(const Duration(milliseconds: 3000), (timer) async {
      if (_latestCameraImage != null && _geminiSession != null && _isGeminiConnected) {
        try {
          final Uint8List? jpegBytes = await compute(convertCameraImageToJpeg, _latestCameraImage!);
          if (jpegBytes != null) {
            final String base64Image = base64Encode(jpegBytes);
            _geminiSession!.sendMessage(LiveClientMessage(
  ...
  Future<void> switchCamera() async {
    ...
    try {
      ...
      final newCamera = _cameras.firstWhere((c) => c.lensDirection != currentLens);
      ...
      await _cameraController!.initialize();
      await _cameraController!.startImageStream((image) => _latestCameraImage = image);
  ...
  Future<void> startRecording() async {
    try {
      ...
      _voiceEngineSubscription = _voiceEngine!.audioChunkStream.listen(
            (audioData) {
          if (_geminiSession != null && _isGeminiConnected && state.isRecording) {
            _geminiSession!.sendMessage(LiveClientMessage(
              realtimeInput: LiveClientRealtimeInput(
                audio: Blob(mimeType: 'audio/pcm;rate=16000', data: base64Encode(audioData)),
              ),
            ));
          }
          final amplitude = computeRMSAmplitude(audioData);
        ...
        onError: (error) => emit(state.copyWith(error: 'Recording error: $error', isError: true)),
      );

      await _voiceEngine!.startRecording();
    } catch (e) {
      emit(state.copyWith(error: 'Failed to start recording: $e', isError: true));
    }
  }
...
Uint8List _amplifyAudio(Uint8List audioData, {double amplificationFactor = 2.0}) {
